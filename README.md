...
[![Generate jlFriCAS Documentation](https://github.com/gvanuxem/jlfricas/actions/workflows/generate_docs.yml/badge.svg)](https://github.com/gvanuxem/jlfricas/actions/workflows/generate_docs.yml)
...
The MCP server is also used by [fricas-vscode](https://github.com/gvanuxem/fricas-vscode) (the Julia interface is not necessary nor the use of a LLM). The Yason Common Lisp library is necessary. For Clozure CL, install it with quicklisp for example. With SBCL on Debian based systems, the `cl-yason` provides it but as of now, unfortunately, `sudo apt install cl-yason`, some dependencies are missing. If you install also `cl-hunchentoot` that will be fixed. If installed it will be used automatically through ASDF.

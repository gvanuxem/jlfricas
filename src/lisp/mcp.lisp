...
(defun make-mcp-tool-schema (name description props &optional required)
...
(defun handle-list-tools (id)
  (let ((result (make-hash-table :test 'equal))
    ...
    (let ((props (make-hash-table :test 'equal))
      ...
      (push (make-mcp-tool-schema "evaluate" "Evaluate a mathematical expression." props '("expression")) tools))
    ...
    (let ((props (make-hash-table :test 'equal))
      ...
      (setf (gethash "name" props) name-prop)
      ...
      (setf (gethash "type" props) type-prop)
      (push (make-mcp-tool-schema "get-documentation" "Get documentation for a FriCAS constructor (Domain, Category, Package) or operation." props '("name" "type")) tools))
    ...
    (let ((props (make-hash-table :test 'equal))
      ...
      (setf (gethash "pattern" props) pattern-prop)
      (push (make-mcp-tool-schema "list-constructors" "List FriCAS constructors (categories, domains, packages) matching a pattern." props '("pattern")) tools))

    ;; describe-constructor
    (let ((props (make-hash-table :test 'equal))
          (name-prop (make-hash-table :test 'equal)))
      (setf (gethash "type" name-prop) "string")
      (setf (gethash "description" name-prop) "The name of the constructor (e.g. 'Integer' or 'Matrix')")
      (setf (gethash "name" props) name-prop)
      (push (make-mcp-tool-schema "describe-constructor" "Describe a FriCAS constructor (domain, package, category), listing its operations and exports." props '("name")) tools))

    ;; list-operations
    (let ((props (make-hash-table :test 'equal))
          (pattern-prop (make-hash-table :test 'equal)))
      (setf (gethash "type" pattern-prop) "string")
      (setf (gethash "description" pattern-prop) "The operation search pattern or name (e.g. 'differentiate' or 'factor')")
      (setf (gethash "pattern" props) pattern-prop)
      (push (make-mcp-tool-schema "list-operations" "List FriCAS operations matching a search pattern." props '("pattern")) tools))

    ;; compile-spad
    (let ((props (make-hash-table :test 'equal))
          (file-prop (make-hash-table :test 'equal)))
      (setf (gethash "type" file-prop) "string")
      (setf (gethash "description" file-prop) "Path to the .spad file to compile")
      (setf (gethash "file" props) file-prop)
      (push (make-mcp-tool-schema "compile-spad" "Compile a Spad source file into FriCAS." props '("file")) tools))

    ;; clear-environment
    (let ((props (make-hash-table :test 'equal))
          (mode-prop (make-hash-table :test 'equal)))
      (setf (gethash "type" mode-prop) "string")
      (setf (gethash "description" mode-prop) "Clear option: 'all' (default), 'completely', 'properties', 'values'")
      (setf (gethash "enum" mode-prop) '("all" "completely" "properties" "values"))
      (setf (gethash "mode" props) mode-prop)
      (push (make-mcp-tool-schema "clear-environment" "Clear workspace variables, properties, or internal interpreter tables." props) tools))

    (setf (gethash "tools" result) (nreverse tools))
    (send-result id result)))
  ...
  (cond
       ...
       (if (not pattern)
           ...
           (let* ((cmd (format nil "listConstructors(\"~A\")$SpadDoc" pattern))
             ...
             (let ((result (make-hash-table :test 'equal))
               ...
               (setf (gethash "content" result) (list content))
               (send-result id result))))))
    ((string= name "describe-constructor")
     (let ((name-arg (gethash "name" params)))
       (if (not name-arg)
           (send-error id -32602 "Missing name parameter")
           (let* ((cmd (format nil "constructorDocumentation(\"~A\")$SpadDoc" name-arg))
                  (output (capture-fricas-output cmd)))
             (let ((result (make-hash-table :test 'equal))
                   (content (make-hash-table :test 'equal)))
               (setf (gethash "type" content) "text")
               ...
               (setf (gethash "content" result) (list content))
               (send-result id result))))))
    ((string= name "list-operations")
     (let ((pattern (gethash "pattern" params)))
       (if (not pattern)
           (send-error id -32602 "Missing pattern parameter")
           (let* ((cmd (format nil "operationDocumentation(\"~A\")$SpadDoc" pattern))
                  (output (capture-fricas-output cmd)))
             (let ((result (make-hash-table :test 'equal))
                   (content (make-hash-table :test 'equal)))
               (setf (gethash "type" content) "text")
    ...
    ((string= name "compile-spad")
     (let ((file (gethash "file" params)))
       (if (not file)
           (send-error id -32602 "Missing file parameter")
           (let* ((cmd (format nil ")compile ~A" file))
                  (output (capture-fricas-output cmd)))
             (let ((result (make-hash-table :test 'equal))
                   (content (make-hash-table :test 'equal)))
               (setf (gethash "type" content) "text")
               ...
               (setf (gethash "content" result) (list content))
               (send-result id result))))))
    ((string= name "clear-environment")
     (let* ((mode (or (gethash "mode" params) "all"))
            (cmd (format nil ")clear ~A" mode))
            (output (capture-fricas-output cmd)))
       (let ((result (make-hash-table :test 'equal))
             (content (make-hash-table :test 'equal)))
         (setf (gethash "type" content) "text")

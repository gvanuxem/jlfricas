...
(defun handle-list-tools (id)
  (let ((result (make-hash-table :test 'equal))
    ...
    (let ((doc-tool (make-hash-table :test 'equal))
      ...
      (setf (gethash "description" type-prop) "Optional entity type ('constructor' or 'operation'). Auto-detected if omitted.")
      ...
      (setf (gethash "required" doc-schema) '("name"))
      ...
      (let ((list-tool (make-hash-table :test 'equal))
        ...
        (let ((compile-tool (make-hash-table :test 'equal))
              (compile-schema (make-hash-table :test 'equal))
              (compile-props (make-hash-table :test 'equal))
              (path-prop (make-hash-table :test 'equal))
              (args-prop (make-hash-table :test 'equal)))
          (setf (gethash "name" compile-tool) "compile-spad")
          (setf (gethash "description" compile-tool) "Compile a SPAD (.spad) source file.")
          (setf (gethash "type" path-prop) "string")
          (setf (gethash "description" path-prop) "The path to the .spad file to compile")
          (setf (gethash "path" compile-props) path-prop)
          (setf (gethash "type" args-prop) "string")
          (setf (gethash "description" args-prop) "Optional compiler options or arguments (e.g., ')quiet', ')noquiet', ')vartype')")
          (setf (gethash "args" compile-props) args-prop)
          (setf (gethash "type" compile-schema) "object")
          (setf (gethash "properties" compile-schema) compile-props)
          (setf (gethash "required" compile-schema) '("path"))
          (setf (gethash "inputSchema" compile-tool) compile-schema)

          (let ((compile-lisp-tool (make-hash-table :test 'equal))
                (compile-lisp-schema (make-hash-table :test 'equal))
                (compile-lisp-props (make-hash-table :test 'equal))
                (lisp-path-prop (make-hash-table :test 'equal))
                (lisp-args-prop (make-hash-table :test 'equal)))
            (setf (gethash "name" compile-lisp-tool) "compile-lisp")
            (setf (gethash "description" compile-lisp-tool) "Compile and load a Common Lisp (.lisp or .lsp) source file using FriCAS system command )read.")
            (setf (gethash "type" lisp-path-prop) "string")
            (setf (gethash "description" lisp-path-prop) "The path to the .lisp or .lsp file to compile and load")
            (setf (gethash "path" compile-lisp-props) lisp-path-prop)
            (setf (gethash "type" lisp-args-prop) "string")
            (setf (gethash "description" lisp-args-prop) "Optional read options (e.g., ')quiet', ')noquiet')")
            (setf (gethash "args" compile-lisp-props) lisp-args-prop)
            (setf (gethash "type" compile-lisp-schema) "object")
            (setf (gethash "properties" compile-lisp-schema) compile-lisp-props)
            (setf (gethash "required" compile-lisp-schema) '("path"))
            (setf (gethash "inputSchema" compile-lisp-tool) compile-lisp-schema)

            (let ((compile-boot-tool (make-hash-table :test 'equal))
                  (compile-boot-schema (make-hash-table :test 'equal))
                  (compile-boot-props (make-hash-table :test 'equal))
                  (boot-path-prop (make-hash-table :test 'equal))
                  (boot-args-prop (make-hash-table :test 'equal)))
              (setf (gethash "name" compile-boot-tool) "compile-boot")
              (setf (gethash "description" compile-boot-tool) "Compile and load a Boot (.boot) source file using FriCAS system command )read.")
              (setf (gethash "type" boot-path-prop) "string")
              (setf (gethash "description" boot-path-prop) "The path to the .boot file to compile and load")
              (setf (gethash "path" compile-boot-props) boot-path-prop)
              (setf (gethash "type" boot-args-prop) "string")
              (setf (gethash "description" boot-args-prop) "Optional read options (e.g., ')quiet', ')noquiet')")
              (setf (gethash "args" compile-boot-props) boot-args-prop)
              (setf (gethash "type" compile-boot-schema) "object")
              (setf (gethash "properties" compile-boot-schema) compile-boot-props)
              (setf (gethash "required" compile-boot-schema) '("path"))
              (setf (gethash "inputSchema" compile-boot-tool) compile-boot-schema)
              (setf (gethash "tools" result) (list tool doc-tool list-tool compile-tool compile-lisp-tool compile-boot-tool))))))
      (send-result id result))))
  ...
  (cond
           ...
           (progn
             ...
             (let ((output (evaluate-expr expr)))
               ...
               (let ((result (make-hash-table :test 'equal))
                 ...
                 (send-result id result)))))))
    ((string= name "compile-spad")
     (let ((path (or (gethash "path" params) (gethash "file" params)))
           (args (or (gethash "args" params) (gethash "options" params))))
       (if (not path)
           (send-error id -32602 "Missing path parameter")
           (progn
             (send-notification "repl/starteval" nil)
             (let* ((trimmed-args (when (and args (stringp args))
                                    (string-trim '(#\Space #\Tab #\Newline #\Return) args)))
                    (opt-str (if (and trimmed-args (not (string= trimmed-args "")))
                                 (if (char= (char trimmed-args 0) #\))
                                     (format nil " ~A" trimmed-args)
                                     (format nil " )~A" trimmed-args))
                                 ""))
                    (cmd (format nil ")compile ~A~A" path opt-str)))
               ;; Echo compilation command to terminal REPL
               (format *error-output* "~A~%" cmd)
               (finish-output *error-output*)
               (let ((output (evaluate-expr cmd)))
                 ;; Echo result to terminal REPL
                 (when (and output (not (string= output "")))
                   (format *error-output* "~A~%" output)
                   (finish-output *error-output*))
                 ...
                 (let ((result (make-hash-table :test 'equal))
                       (content (make-hash-table :test 'equal)))
                   (setf (gethash "type" content) "text")
                   (setf (gethash "text" content) (or output ""))
                   (setf (gethash "content" result) (list content))
                   (send-result id result))))))))
    ((string= name "compile-lisp")
     (let ((path (or (gethash "path" params) (gethash "file" params)))
           (args (or (gethash "args" params) (gethash "options" params))))
       (if (not path)
           (send-error id -32602 "Missing path parameter")
             ...
             (let* ((trimmed-args (when (and args (stringp args))
                    ...
                    (opt-str (if (and trimmed-args (not (string= trimmed-args "")))
                                 (if (char= (char trimmed-args 0) #\))
                                     ...
                                     (format nil " )~A" trimmed-args))
                                 ""))
                    (cmd (format nil ")read ~A~A" path opt-str)))
               ;; Echo command to terminal REPL
               (format *error-output* "~A~%" cmd)
               (finish-output *error-output*)
               (let ((output (evaluate-expr cmd)))
                 ;; Echo result to terminal REPL
    ...
    ((string= name "get-documentation")
     (let ((entity-name (gethash "name" params))
       ...
       (if (not entity-name)
           (send-error id -32602 "Missing name parameter")
           (let* ((doc (get-fricas-documentation entity-name entity-type))
                  (result (make-hash-table :test 'equal))
                  (content (make-hash-table :test 'equal)))
             (setf (gethash "type" content) "text")
             (setf (gethash "text" content) doc)
             (setf (gethash "content" result) (list content))
             (send-result id result)))))
...
(defun fricas-constructor-p (word)
  "Check if WORD is a known FriCAS constructor or abbreviation."
  (handler-case
      (let ((sym (intern word "BOOT")))
        (or (and (fboundp 'boot::|constructor?|) (boot::|constructor?| sym))
            (and (fboundp 'boot::|abbreviation?|) (boot::|abbreviation?| sym))))
    (error () nil)))

(defun query-fricas-doc (fricas-func entity-name)
  (let* ((cmd (format nil "~A(\"~A\")$SpadDoc" fricas-func entity-name))
         (output (capture-fricas-output cmd)))
    (clean-fricas-doc-output output)))

(defun get-fricas-documentation (entity-name &optional entity-type)
  "Fetch documentation for ENTITY-NAME. If ENTITY-TYPE is not specified,
   automatically detects whether it is a constructor or operation."
  (cond
    ((equal entity-type "constructor")
     (query-fricas-doc "constructorDocumentation" entity-name))
    ((equal entity-type "operation")
     (query-fricas-doc "operationDocumentation" entity-name))
    (t
     ;; Auto-detect: prioritize constructor if known constructor or starts with uppercase
     (let ((is-constructor (or (fricas-constructor-p entity-name)
                               (and (> (length entity-name) 0)
                                    (upper-case-p (char entity-name 0))))))
       (if is-constructor
           (let ((doc (query-fricas-doc "constructorDocumentation" entity-name)))
             (if (and doc (not (string= (string-trim '(#\Space #\Newline #\Return) doc) "")))
                 doc
                 (query-fricas-doc "operationDocumentation" entity-name)))
           (let ((doc (query-fricas-doc "operationDocumentation" entity-name)))
             (if (and doc (not (string= (string-trim '(#\Space #\Newline #\Return) doc) "")))
                 doc
                 (query-fricas-doc "constructorDocumentation" entity-name))))))))
...
(defun handle-repl-get-doc-from-word (id params)
  (let ((word (gethash "word" params))
    ...
    (if (not word)
        ...
        (let ((doc (get-fricas-documentation word entity-type)))
          (setf (gethash "result" result) doc)
